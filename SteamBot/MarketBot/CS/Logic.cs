        ...
        private void SetOrderForUnstickered() {
            while (true) {
                if (needOrderUnstickered.Count > 0) {
                    ...
                    if (price > 9000 && curPrice < price * 0.79 && !blackList.Contains(top.i_market_hash_name)) {
        ...
        public bool WantToBuy(NewItem item) {
            ...
            lock (DatabaseLock)
            {
                ...
                if (item.ui_price < 25000 && prices.Count >= 6 &&
                    item.ui_price < 0.825 * prices[2] && !blackList.Contains(item.i_market_name) &&
                    salesHistory.cnt >= MINSIZE &&
                    prices[2] < dataBase[item.i_market_name].median * 1.225 && prices[2] - item.ui_price > 400)
                {
